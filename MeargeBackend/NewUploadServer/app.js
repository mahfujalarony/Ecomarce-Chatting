...
function isUploadManagedPath(url = "") {
  const pathname = String(url || "").split("?")[0] || "/";
  return (
    pathname.startsWith("/public") ||
    pathname === "/create-folder" ||
    pathname.startsWith("/upload/") ||
    pathname.startsWith("/delete/chat/") ||
    pathname.startsWith("/delete-folder/chat/")
  );
}

async function handleUploadRequest(req, res, next) {
  if (!isUploadManagedPath(req.url)) {
  ...
  res.setHeader("Access-Control-Allow-Origin", requestOrigin || config.BASE_URL);
...
function startStandaloneServer() {
  const server = http.createServer((req, res) => handleUploadRequest(req, res));
