...
import { CHAT_BASE_URL, CHAT_SOCKET_PATH } from "../../config/env";
...
const GuestSupportChat = () => {
  ...
  useEffect(() => {
    if (!token) return;
    const s = io(CHAT_API, { auth: { token }, path: CHAT_SOCKET_PATH, transports: ["websocket"] });
    setSocket(s);
    return () => {
      s.disconnect();
      setSocket(null);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        ...
        if (status === 401 || status === 403 || status === 404 || msg.includes("conversation not found")) {
          clearGuestSession();
        }
      });
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
          ...
          {error && <Alert type="error" title={error} showIcon style={{ margin: 8 }} />}
            ...
            <>
              ...
              {isBlocked && (
                <Alert
                  type="warning"
                  showIcon
                  title="This chat is blocked by admin. You cannot send new messages now."
                  style={{ margin: 8 }}
                />
              )}
