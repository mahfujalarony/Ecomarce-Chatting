...
import { API_BASE_URL, HOME_CACHE_TTL_SECONDS } from "../../config/env";
...
const getPriceMeta = (product) => {
  ...
  return { price, oldPrice, hasOldPrice, discountPct };
};
...
const BannerHeroSkeleton = () => (
  <div className="relative h-[200px] sm:h-[290px] md:h-[340px] lg:h-[360px] xl:h-[380px] overflow-hidden">
        ...
        {newArrivalsLoading ? (
          ...
          <div className="flex overflow-x-auto gap-4 pb-6 scrollbar-hide snap-x">
            {trendingProducts.map((product, idx) => {
              const { price, oldPrice, hasOldPrice, discountPct } = getPriceMeta(product);
              return (
              <div
                key={product.id}
                  ...
                  <div className="mt-auto flex items-start justify-between gap-2">
                    <div className="flex flex-col">
                      <div className="flex items-center gap-1.5">
                        ...
                        {hasOldPrice ? <span className="text-[10px] font-semibold text-emerald-600">-{discountPct}%</span> : null}
                      </div>
                      {hasOldPrice ? <span className="text-[10px] text-gray-400 line-through">${oldPrice.toFixed(2)}</span> : null}
                    </div>
