...
const Navbar = ({ collapsed, setCollapsed, openDrawer, isMobile, categories = [], catLoading = false }) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("scroll", closeFloatingPanels);
  }, []);
          ...
          <div className="flex items-center gap-2 md:gap-3">
            {isMobile ? (
              ...
              <Dropdown
                ...
                trigger={["click"]}
                open={desktopCategoryOpen}
                onOpenChange={setDesktopCategoryOpen}
                getPopupContainer={(trigger) => trigger?.parentElement || navbarRef.current || document.body}
                menu={{
